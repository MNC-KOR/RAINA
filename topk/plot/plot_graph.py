...
#for entry in [1000,2000,3000,4000,5000,6000,7000,8000,9000,10000,11000,12000,13000,14000,15000]: #,5000]: # FIXME : modify just this line
    ...
    for i in range (1,num_exp+1):
        dist = 'z'
        t = 'topk'
        f = open('/home/mnc/p4-dev/topk/logs/mininet/daiet/daiet-fat_tree_reg3000-{}-{}-{}-{}-{}.csv'.format(dist,p,num,entry,i), 'r') #daiet-fat_tree-z-1.1-1000-5000-1
        opened_file = f.readlines()
        a = opened_file[-1].split(',')[1]
        b = float(opened_file[-1].split(',')[3]) /10
        print(a)
        print(b)
        print((1-float(a)/float(b)))
        # result_daiet[str(entry)].append(1-int(a)/(12*num))
        
        result_daiet[str(entry)].append(1-float(a)/float(b))
        f.close()
...
############
    for i in range (1,num_exp+1): 
        dist = 'z'
        t = 'topk'
        f = open('/home/mnc/p4-dev/topk/logs/mininet/topk/topk-fat_tree_reg3000-{}-{}-{}-{}-{}.csv'.format(dist,p,num,entry,i), 'r') #topk-fat_tree-z-1.1-1000-1000-1
        opened_file = f.readlines()
        a = opened_file[-1].split(',')[1]
        b = float(opened_file[-1].split(',')[3]) / 10
        print(a)
        ...
        result_topk[str(entry)].append(1-float(a)/float(b))
        f.close()
...
plt.savefig('graph-fat_tree_reg3000_20000_subin_new.png')