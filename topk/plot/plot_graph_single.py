...
for entry in [1000, 2000]: # FIXME : modify just this line
    ...
    for i in range (1,2):
        for case in [1,2,3]:
            ...
            t = 'daiet'
            dist = 'z'
            f = open('/home/mnc/p4-dev/topk/logs/mininet/daiet/daiet-single-{}-{}-{}-{}-{}-{}.csv'.format(num,dist,entry,p,case,i), 'r')
            opened_file = f.readlines()
            a = opened_file[-1].split(',')[1]
            result_daiet[str(entry)].append(1-int(a)/(num))
            f.close()

            # f = open('/home/mnc/p4-dev/topk/logs/mininet/daiet/{}-single-{}-{}-{}-{}-{}.csv'.format(t,num,dist,entry,p2,i), 'r')
            # opened_file = f.readlines()
            # a = opened_file[-1].split(',')[1]
            ...
            # dist = 'u'
            # # f = open('/home/mnc/p4-dev/topk/logs/mininet/{}-single-{}-{}-{}-{}.csv'.format(t,num,'',entry,i), 'r') 
            # f = open('/home/mnc/p4-dev/topk/logs/mininet/daiet/{}-single-{}-{}-{}-{}.csv'.format(t,num,dist,entry,i), 'r') 
            # opened_file = f.readlines()
            # a = opened_file[-1].split(',')[1]
            # print(a)
            ...
            t = 'topk'
            dist = 'z'
            f = open('/home/mnc/p4-dev/topk/logs/mininet/topk/topk-single-{}-{}-{}-{}-{}-{}.csv'.format(num,dist,entry,p,case,i), 'r')
            opened_file = f.readlines()
            a = opened_file[-1].split(',')[1]
            result_topk[str(entry)].append(1-int(a)/(num))
            f.close()

            # f = open('/home/mnc/p4-dev/topk/logs/mininet/topk/{}-single-{}-{}-{}-{}-{}.csv'.format(t,num,dist,entry,p2,i), 'r')
            # opened_file = f.readlines()
            # a = opened_file[-1].split(',')[1]
            ...
            # dist = 'u'
            # # f = open('/home/mnc/p4-dev/topk/logs/mininet/{}-single-{}-{}-{}-{}.csv'.format(t,num,'',entry,i), 'r')
            # f = open('/home/mnc/p4-dev/topk/logs/mininet/topk/{}-single-{}-{}-{}-{}.csv'.format(t,num,dist,entry,i), 'r') 
            # opened_file = f.readlines()
            # a = opened_file[-1].split(',')[1]
            # print(a)
...
plt.savefig('graph-single_test.png')
