...
for d_max in [args.entry]: #, 10000,15000,20000,25000,30000]:
    ...
    if dist == 'u':
        ...
        if args.sort == 1:
            ...
            new_list = sorted(count.items(), reverse=False, key=lambda count:count[1])
            print(new_list)
            # 


        with open('/home/mnc/p4-dev/topk/packets/dataset/{}_dist_{}_{}'.format(dist,str(d_max),num_of_data), 'w') as f:
    ...
    if dist == 'z':
        ...
        if args.sort == 1 or args.sort ==2:
            ...
            # print(count)
            if args.sort == 1 :
                sorted_count = sorted(count.items(), reverse=False, key=lambda count:count[1])
            ...
            # print(n[1])
            if args.index != '':
                f = open('/home/mnc/p4-dev/topk/packets/dataset/{}_dist_{}_{}_{}_{}'.format(dist,p,str(d_max),num_of_data, args.index) , 'w')
            else:
                f = open('/home/mnc/p4-dev/topk/packets/dataset/{}_dist_{}_{}_{}'.format(dist,p,str(d_max),num_of_data) , 'w')
            for i in range(0,num_of_data,10):
                keys = str(n[i]) + ' ' + str(n[i + 1]) + ' ' + str(n[i + 2]) + ' ' + str(n[i + 3]) + ' ' + str(n[i + 4]) + ' ' 
                keys += str(n[i + 5]) + ' ' + str(n[i + 6]) + ' ' + str(n[i + 7]) + ' ' + str(n[i + 8]) + ' ' + str(n[i + 9]) + '\n'
                f.write(keys)
